...
前端为静态资源，用 Python 内置 HTTP 服务跑在项目根目录。

- **修改代码、样式或本仓库内资源后**：应重启上述服务，避免浏览器/缓存与旧进程状态不一致。
- 一键重启：`bash scripts/restart-dev-server.sh` 或 `npm run restart-server`
- 端口：**9342**（`127.0.0.1`）、**8765**（`0.0.0.0`，局域网）、**8081**（`127.0.0.1`）

（协助开发时：在完成对本项目的文件修改后，执行重启脚本或等效命令。）
